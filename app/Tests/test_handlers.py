...
from app.Api.Telegram_Api.Bot import (
    ...
    Form, pending_codes, notification_about_deadline, EXTERNAL_API_URL, headers, bot
)
...
async def test_process_email_valid_domain(fake_msg, fsm, monkeypatch):
    ...
    monkeypatch.setattr(bot_module.random, "randint", lambda *args, **kwargs: 123456)
...
@pytest.mark.asyncio
async def test_process_verification_correct_code_update_existing(fake_msg, fsm, monkeypatch):
    ...
    monkeypatch.setattr(bot_module.requests, "patch", lambda *args, **kwargs: MagicMock())
...
@pytest.mark.asyncio
async def test_process_verification_correct_code_new_supervisor_branch(fake_msg, fsm, monkeypatch):
...
async def test_notification_about_deadline_single_iteration(monkeypatch):
    ...
    m = {'id': 1, 'deadline': (now + datetime.timedelta(hours=1)).isoformat()+'Z', 'status':'in process', 'notified':'in_1_hour', 'title':'t','description':'d'}
    ...
    patched = []
    def fake_patch(url, **kw): patched.append(url)
